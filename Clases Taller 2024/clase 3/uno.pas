...
a. Implementar un modulo que almacene informacion de socios de un 
club en un arbol binario de busqueda. De cada socio se debe almacenar numero de socio, 
nombre y edad. La carga finaliza con el numero de socio 0 y el arbol 
debe quedar ordenado por numero de socio. La informacion de cada socio debe generarse
aleatoriamente.
b. Una vez generado el arbol, realice modulos independientes 
que reciban el arbol como parametro para: 
    i. Informar los datos de los socios en orden creciente.
    ii. Informar los datos de los socios en orden decreciente.
    iii. Informar el número de socio con mayor edad. Debe invocar a un 
    módulo recursivo que retorne dicho valor.
    iv. Aumentar en 1 la edad de los socios con edad impar e informar 
    la cantidad de socios que se les aumento la edad.
    vi. Leer un nombre e informar si existe o no existe un socio 
    con ese nombre. Debe invocar a un módulo recursivo que reciba el nombre 
    leído y retorne verdadero o falso.
    vii. Informar la cantidad de socios. Debe invocar a un módulo recursivo 
    que retorne dicha cantidad.
    viii. Informar el promedio de edad de los socios. Debe invocar a un 
    módulo recursivo que retorne el promedio de las edades de los socios.
...
{ Informar el numero de socio con mayor edad. Debe invocar a un modulo 
...
var
  ...
  begin
    if (a=nil) then  //caso base no hay lista, no hay nombre
      existe:=false
    else if (a <> nil) and (nombre=a^.dato.nombre)then //hay lista y coincide
      existe:=true
    else begin
      existe:= existe(a^.HI,nombre); // buscamos en el subarbol izquierdo
      if not existe then
        existe:=existe(a^.HD,nombre); // sino lo encontramos lo buscamos en el derecho
    end;  
  end;

begin
  writeln;
  writeln ('----- Informar Existencia de un Nombre de Socio ----->');
  writeln;
  writeln ('Nombre a buscar: ');Readln(nombre);
  if (existe(a,nombre)) then
    writeln('El nombre se encuentra en la lista')
  else 
    writeln('Lo sentimos,el nombre NO se encuentra en la lista');
  writeln;
  writeln ('//////////////////////////////////////////////////////////');
  writeln;
end;
...
begin
  writeln;
  writeln ('----- Cantidad de socios total ----->');
  writeln;
  writeln ('Cantidad: ', Cantidad(a));
  writeln;
  writeln;
  writeln ('//////////////////////////////////////////////////////////');
  writeln;
...
procedure InformarPromedioDeEdad(a:arbol);
  ...
  function SumaEdades(a:arbol):real;
  begin
    if (a=nil) then
      SumaEdades:=0;
    if (a<>nil) then
...
begin
  writeln;
  writeln ('----- El Promedio de edades de los socios ----->');
  writeln;
  promedio:=SumaEdades(a)/Cantidad(a);
